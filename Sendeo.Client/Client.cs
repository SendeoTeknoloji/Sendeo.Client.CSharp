...
using Sendeo.Client.Models;
using Sendeo.Client.Models.Request;
using Sendeo.Client.Models.Response;
using Sendeo.Client.Utilities;
using System.Net.Http.Headers;
using System.Net.Mime;
using System.Text;

namespace Sendeo.Client;
...
public class Client
{
    ...
    public async Task<ResponseDataOfTokenResponseData> LoginAsync(LoginRequest request)
    {
        ...
        try
        {
            ...
            var responseStream = await response.Content.ReadAsStreamAsync() ?? throw new ApiException("Giriş yapma adresinden boş cevap döndürüldü.");
    ...
    public async Task<ResponseWrapper<CancelDeliveryResponse>> CancelDeliveryAsync(CancelDeliveryRequest request, string? token)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.CustomerReferenceNo))
        {
            queryValues.Add("referenceNo", request.CustomerReferenceNo);
        }

        return await SendPostRequestAsync<CancelDeliveryResponse>("api/Cargo/CANCELDELIVERY", null, token, queryValues);
    }
    ...
    /// <returns> Sendeo Entegrasyon'dan dönen cevabın <typeparamref name="T"/> nesnesine dönüştürülmüş biçimini döndürür. </returns>
    /// <exception cref="ApiException"> API'den bir hata gelirse fırlatılır. </exception>
    private async Task<ResponseWrapper<T>> SendPostRequestAsync<T>(string uri, object? request, string? token = null, IDictionary<string, string>? arguments = null)
    {
        var result = new ResponseWrapper<T>();
        ...
        try
        {
            ...
            {
                var requestAsJson = JsonConvert.SerializeObject(request);
                requestContent = new StringContent(requestAsJson, Encoding.UTF8, MediaTypeNames.Application.Json);
            }
            ...
            var response = await _httpClient.PostAsync(requestAddress, requestContent);
            var responseContent = await response.Content.ReadAsStringAsync();
            var responseStream = await response.Content.ReadAsStreamAsync();
